...
class AddCustomer(tk.Frame):
    def __init__(self, master, tempCursorInit):
        ...
        self.master = master
        self.master.title("Add Customer")
        ...
        self.pincode_entry.grid(row=6, column=1, padx=10, pady=5)

        # Submit Button
        self.submit_button = tk.Button(
            self.master, text="Submit", command=self.submit_customer)
        self.submit_button.grid(
            row=7, column=0, columnspan=2, padx=10, pady=10)

    def submit_customer(self):
        # Getting Inputs from all entries and storing into Temporary Variables
        ...
        # Setting Constraints on all Variables
        if not first_name or not second_name or not phone or not address_one or not address_two or not pincode:
            ...
            # Finally Inserting values to the Database
            Processes.Cursor_Init.insertHuman(
                self.tempCursorInit, "Customer", first_name, second_name, phone, address_one, address_two, pincode)
